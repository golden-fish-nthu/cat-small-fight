...
void UI::draw() {
    ...
    al_draw_textf(FC->courier_new[FontSize::MEDIUM], al_map_rgb(0, 0, 0), game_field_length - 400,
                  DC->window_height - tower_img_bottom_padding, ALLEGRO_ALIGN_LEFT, "coin: %5d", player_coin);
    const int& player_hp = DC->player->HP;
    al_draw_textf(FC->courier_new[FontSize::MEDIUM], al_map_rgb(0, 0, 0), game_field_length - 400,
                  DC->window_height - tower_img_bottom_padding + 20, ALLEGRO_ALIGN_LEFT, "HP: %5d", player_hp);
    // 繪製塔商店項目
    for (auto& [bitmap, p, price] : tower_items) {
        int w = al_get_bitmap_width(bitmap);
        int h = al_get_bitmap_height(bitmap);
        al_draw_bitmap(bitmap, p.x, p.y, 0);
        al_draw_rectangle(p.x - 1, p.y - 1, p.x + w + 1, p.y + h + 1, al_map_rgb(0, 0, 0), 1);
        if (price > DC->player->coin) {
            // 金額不足，顯示半透明遮罩
            al_draw_filled_rectangle(p.x, p.y, p.x + w, p.y + h, al_map_rgba(50, 50, 50, 128));
        }
        al_draw_textf(FC->courier_new[FontSize::MEDIUM], al_map_rgb(0, 0, 0), p.x + w / 2, p.y + h,
